...
//int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (rsz == 0){
			 if (ferror(infile)) { printf("[PH264][Error][%s]:!! Error: read failed: %s \n", __func__, strerror(errno)); break; }
        ...
        while (find_nal_unit(p, sz, &nal_start, &nal_end) > 0)
        {
            if ( opt_verbose > 0 )
            {
               fprintf( h264_dbgfile, "[PH264][Debug][%s]:!! Found NAL at offset %lld (0x%04llX), size %lld (0x%04llX) \n", __func__,
            ...
            if ( opt_verbose > 0 )
            {
                fprintf( h264_dbgfile, "[PH264][Debug]:XX ");
        ...
        /*如果缓冲区中没有NALs，就丢弃它*/
        if (p == buf) {
            printf("[PH264][Error][%s]:!! Did not find any NALs between offset %lld (0x%04llX), size %lld (0x%04llX), discarding \n", __func__, 
                   (long long int)off, 
                   (long long int)off, 
